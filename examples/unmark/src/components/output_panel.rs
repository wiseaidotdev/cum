                ...
                if let Some(r) = &props.result {
                    <div class="flex items-center gap-1.5 md:gap-2">
                        if matches!(r.kind, crate::types::MediaKind::Text) {
                            <button
                                class="um-btn-ghost text-xs px-2.5 md:px-3"
                                onclick={on_copy}
                                id="btn-copy"
                            >
                                if *copied {
...
fn render_output(props: &OutputPanelProps, _copied: bool) -> Html {
    match &props.result {
        None => html! {
            <div
                class="flex-1 flex flex-col items-center justify-center gap-3 text-um-muted select-none py-8"
            >
                <div
                    class="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-um-elevated flex items-center justify-center"
                >
        ...
        Some(r) => html! {
            <>
                ...
                <div class="flex-1 min-h-0 animate-fade-in-up">{ render_content(r) }</div>
            </>
        },
    }
}
...
fn render_content(r: &CleanResult) -> Html {
    match &r.kind {
        ...
        _ => {
            ...
            html! {
                <pre
                    class="text-xs md:text-sm text-um-text leading-relaxed whitespace-pre-wrap break-words font-mono bg-um-elevated rounded-xl p-3 md:p-4 min-h-[120px] md:min-h-[200px] overflow-auto"
                >
                    { text }
                </pre>
            }
